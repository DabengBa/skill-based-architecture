...
3. **Inspect first** — gather repo evidence before questioning: similar features, entry points, config, scripts, tests, and current docs. **If this inspection is likely to read > 20 files**, dispatch an `explore` subagent per `workflows/subagent-driven.md` Phase 1 contract format; have it return a structured summary (entry points + relevant files + current conventions in this area) instead of pulling all of that into the planning context. Skip the dispatch for narrow inspections or degraded harnesses — do it inline.
...
7. **Prepare execution context** — make sure `prd.md` (or files clearly linked from it) gives the implementer and the reviewer everything they need to read first. No required filename or format for this — write it however it stays readable. **If implementation is multi-hour or touches independent regions**, the implementer is often a separate `implement` subagent (or several) dispatched per `workflows/subagent-driven.md`; in that case, `prd.md`'s reading list is what each subagent's contract `Inputs` field will point to. Plan the cut-points (which files each implementer owns, which are shared, which are forbidden) now while the scope is fresh.
